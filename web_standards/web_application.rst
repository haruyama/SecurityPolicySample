...
* サービスの開発者は, CSRF対策用の秘密情報に以下の情報を用いる必要がある.
  ...
  * セッションIDのハッシュ値 
    ...
    * 参考: CSRF対策用トークンの値にセッションIDそのものを使ってもいい時代なんて、そもそも無かった - https://gist.github.com/mala/9086206

  * 十分なエントロピーを持つ乱数源からの値

    * GNU/Linux の場合は /dev/urandom が利用できる. random(4) 参照.
    * 使用可能なエントロピーを監視し枯渇したサーバをサービスから外す体制があると望ましい.

ただし, フレームワークなどで用意されている方法があればそれを利用してもよい.  フレームワークが利用する秘密情報を利用する場合は, 十分なエントロピーが含まれているか確認すること.
